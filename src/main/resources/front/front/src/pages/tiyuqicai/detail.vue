		...
		<div class="attr" :style='{"padding":"20px","borderColor":"#52b646","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"3px","display":"flex","position":"relative","borderStyle":"dotted dashed solid double","justifyContent":"center"}'>
			...
			<div class="info" :style='{"width":"50%","padding":"10px","margin":"0 2% 0 0px","background":"#fff","order":"2"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"radial-gradient(circle, rgba(132,218,110,1) 0%, rgba(63,187,33,1) 81%, rgba(82,182,70,1) 100%)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="isAuth('tiyuqicai','借用')" @click="onAcross('qicaijieyong','','','')" type="warning">借用</el-button>
				</div>
			</div>
  ...
  export default {
    ...
    methods: {
      ...
      onAcross(acrossTable,crossOptAudit,statusColumnName,tips,statusColumnValue){
        localStorage.setItem('crossTable',`tiyuqicai`);
      ...
      getDiscussList(page) {
        this.$http.get('discusstiyuqicai/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusstiyuqicai/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
