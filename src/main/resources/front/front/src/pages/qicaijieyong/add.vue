...
<div :style='{"padding":"20px 20px 40px","margin":"10px auto 0","borderColor":"#52b646","borderRadius":"12px","background":"#fff","borderWidth":"3px","width":"1200px","position":"relative","borderStyle":"solid dashed double double"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="器材名称" prop="qicaimingcheng">
            <el-input v-model="ruleForm.qicaimingcheng" 
                placeholder="器材名称" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="器材类型" prop="qicaileixing">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="借用时长" prop="jieyongshizhang">
            <el-input v-model="ruleForm.jieyongshizhang" 
                placeholder="借用时长" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="借用时间" prop="jieyongshijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="借用说明" prop="jieyongshuoming">
            <el-input v-model="ruleForm.jieyongshuoming" 
                placeholder="借用说明" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('qicaijieyong/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('qicaijieyong/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post(table+`/update`,obj).then(res => {});
                          this.$http.post('qicaijieyong/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
